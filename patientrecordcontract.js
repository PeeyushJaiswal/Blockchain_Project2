...
class PatientRecordContract extends Contract {
    ...
    */
    ...
    async createPatientRecord(ctx,username,name,dob,gender,blood_type){
        let pRec = PatientRecord.createInstance(username,name,dob,gender,blood_type);
        //TASK 0
        // Add patient record by calling the method in the PRecordList
        await ctx.patientRecordList.addPRecord(pRec);
        // throw new Error()
        return pRec.toBuffer();
    }

    async getPatientByKey(ctx, username, name){
        let pRecKey = PatientRecord.makeKey([username,name]);
        //TASK-1: Use a method from patientRecordList to read a record by key
        let pRec = await ctx.patientRecordList.getPRecord(pRecKey);
        return JSON.stringify(pRec)
    }
    ...
    async updateCheckupDate(ctx,username,name,last_checkup_date){
        ...
        let pRec = await ctx.patientRecordList.getPRecord(pRecKey);
        pRec.set_last_checkup_date(last_checkup_date);
        await ctx.patientRecordList.updatePRecord(pRec);
        return JSON.stringify(pRec)
    }
